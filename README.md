...
#### Add 2FA after signing up

Giving the fact that two-factor authentication hook is set on Warden's `after_authentication` event, it is not triggered after signing up.

The `after_set_user` event is called a lot of times and can not be used for the hook, so we need to override `RegistrationController#sign_up` method to set it up.

For that:
- Generate a custom `RegistrationsController` with Devise if you don't have one already
- Set it in `devise_for` options in the routes
- Override the `sign_up` method to ad the 2FA process
...
class Users::RegistrationsController < Devise::RegistrationsController
  ...
  def sign_up(resource_name, resource)
    # TwoFactorAuthentication triggers the 2FA process on Warden's event "after_authentication"
...
To add them, generate a migration such as:
    ...
    $ rails g migration AddTwoFactorDeliveryMethodToUsers direct_otp_delivery_method:string
...
If your default delivery method is not `:mobile_phone` and/or that you need a conditional fallback, override the `direct_otp_default_delivery_method` method to add your custom logic.
